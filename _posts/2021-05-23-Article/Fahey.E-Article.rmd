...
The main descriptive statistics for each scale can be found in table 1 below:  
...
kable(
  ...
  footnote(
    general_title = "Note.",
    general = "Standard deviations are in parentheses. SHS = Subjective Happiness Scale; SWLS = Satisfaction with Life Sacle; MotHap = Happy Motivation Subscale; MotSad = Sad Motivation Subscale; Motref = Refletive Motivation Subscale.",
    threeparttable = TRUE,
...
A network graph, shown in fig \@ref(fig:net1) is used to display the connectedness of the scales used in the current study. As expected, life satisfaction and happiness form a connection. The three subscales of reasons to visit a place form a strong connection.  
...
```{r modsel, echo=FALSE, fig.cap= "Conceptual structural equation model of the present study"}
...
A structural equation model (fig \@ref(fig:modsel)) is run in this study to analyse reasons to visit a meaningful place against life satisfaction and happiness. The (ML) is employed, which presumes multivariate normality of the variables. As stated previously and depicted in fig \@ref(fig:plots), the motivations to visit a place subscales were not normally distributed, so a bootstrap adjustment was fitted to the model. This runs the same analysis 1,000 times to get an average which creates more reliable and robust results, particularly when considering non-normality.  

## Results

The structural equation model (SEM) is fully saturated (CFI = 1) in the initial model. A second model is run to determine if fit differs by gender. The fit does not differ across the two models (CFI = 1 for both). This implies that gender does not impact the model. Regression paths are identical between groups, therefore the model can be confidently collapsed together. 

Table 2 shows the regression estimate paths of each variable. Only one path is significant, between happiness and happy motivation to visit a place (B = 0.139, *p*=0.010). Life satisfaction and happy motivation to visit a place is near significant (B = 0.104, *p*=0.070), this is likely due to the correlation between happiness and life satisfaction shown in fig \@ref(fig:corr). 
...
# to report the above model. FOR RESULTS
...
parameterestimates(sem_res_strap) %>%
  ...
  papaja::apa_table(.,
                    ...
                    note = "swls = Life Satisfaction; shs = Happiness; motsad = Sad motivation subscale; mothap = Happy motivation subscale; motref = Reflective motivation subscale")
...
Below are
five statements that you may agree or disagree with. Using the 1 -- 7 scale below indicate your agreement with each item. Please be open and honest in your responding. 
...
How important are the following situations as reasons to visit the place in the
photo? (Rating scale from 1 (not at all) to 7 (very important))
...
The exploratory graph analysis (EGA) in appendix \@ref(fig:appen1) presents items from the reason to visit a place scale as three separate constructs.
