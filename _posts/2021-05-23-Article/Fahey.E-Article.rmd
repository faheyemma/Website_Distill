...
```
```{r plots, echo=FALSE, fig.cap=fig$cap("figs", "Distribution of Scales.")}
plot(p1 + p2 + p3 + p4 + p5)
```

A distribution of the scales is shown in figure`r fig$ref("figs")`
...
A table of the main demographics can be found here: 
...
# selecting what we want to correlate 
df3 <- df %>%
  select(age_clean, swls, shs, motsad, mothap, motref)
head(df3) # checking the variables 

cortab <- round(cor(df3),2)
cortab

# make clearner - hide the upper triangle (replication)
upper.tri(cortab)

upper <- cortab
upper[upper.tri(cortab)] <- ""
...
The below is two graphs that show the connectedness of the scales used in the study. We can see that life satisfaction and happiness form a connection, as expected. The ega graph shows that they are still seperate constructs however. 

But the motivations subscales show that all 3 subscales are viewed as one construct, they are not independant and are strongly related. 
...
So - we have a new netowrk graph that teasing out any connectinos between the importance scale: 
